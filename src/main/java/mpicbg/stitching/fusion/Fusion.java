...
public class Fusion {
	 ...
	 * @param subpixelResolution - if there is no subpixel resolution, we do not
	 *                           need to convert to float as no interpolation is
	...
	public static <T extends RealType<T> & NativeType<T>> ImagePlus fuse(final T targetType, final ArrayList<ImagePlus> images, final ArrayList<InvertibleBoundable> models, final int dimensionality,
		...
		for (int t = 1; t <= numTimePoints; ++t) {
			for (int c = 1; c <= numChannels; ++c) {
				IJ.showStatus("Fusing time point: " + t + " of " + numTimePoints + ", " + "channel: " + c + " of " + numChannels + "...");
				...
				if (subpixelResolution) {
					...
					// for linear interpolation we want to mirror, otherwise we get black areas at
					// the first and last pixel of each image
					final InterpolatorFactory<FloatType, RandomAccessible<FloatType>> interpolatorFactory = new NLinearInterpolatorFactory<FloatType>();// new
																																						// OutOfBoundsStrategyMirrorFactory<FloatType>()
																																						// );
					...
					final InterpolatorFactory<FloatType, RandomAccessible<FloatType>> interpolatorFactoryFloat = new NearestNeighborInterpolatorFactory<FloatType>();// new
																																										// OutOfBoundsStrategyValueFactory<FloatType>()
																																										// );
					final InterpolatorFactory<UnsignedShortType, RandomAccessible<UnsignedShortType>> interpolatorFactoryShort = new NearestNeighborInterpolatorFactory<UnsignedShortType>();// new
																																																// OutOfBoundsStrategyValueFactory<UnsignedShortType>()
																																																// );
					final InterpolatorFactory<UnsignedByteType, RandomAccessible<UnsignedByteType>> interpolatorFactoryByte = new NearestNeighborInterpolatorFactory<UnsignedByteType>();// new
																																															// OutOfBoundsStrategyValueFactory<UnsignedByteType>()
																																															// );
	...
	protected static <T extends RealType<T>> void fuseBlock(final Img<T> output, final ArrayList<? extends ImageInterpolation<? extends RealType<?>>> input, final double[] offset,
		...
		final List<ClassifiedRegion> tiles = buildTileList(numImages, numDimensions, transform, input, offset);
		...
		final List<ArrayList<RealRandomAccess<? extends RealType<?>>>> interpolators = new ArrayList<ArrayList<RealRandomAccess<? extends RealType<?>>>>();
		...
		for (int tileIndex = 0; tileIndex < tiles.size(); tileIndex++) {
			...
			threadChunks.clear();
			threadChunks.addAll(SimpleMultiThreading.divideIntoChunks(dimensionSize, threads.length));
	...
	/**
	 * Helper method to generate a list of all non-overlapping tiles. The dimensions
	 * and position of each tile are based on the input and offset parameters. From
	 * these, all overlaps are computed, and ultimately a list of
	 ...
	 */
	private static List<ClassifiedRegion> buildTileList(int numImages, int numDimensions, ArrayList<InvertibleBoundable> transform,
		...
		for (int i = 0; i < numImages; ++i) {
			...
			for (int d = 0; d < numDimensions; ++d) {
				min[d] -= offset[d];
				// Sets each interval to the smallest possible, by rounding the min up and the
				// max down
				Interval ival = new Interval((int) Math.ceil(min[d]), (int) Math.floor(min[d] + input.get(i).getImg().dimension(d) - 1));
		...
		while (!rawTiles.isEmpty()) {
			...
			// TODO instead of iterating we really should use a look up tree structure, as
			// this
	...
	/**
	 * Takes two overlapping regions and deconstructs them into a set of
	 * non-overlapping regions. Each resulting region gains the classification(s) of
	 * its parent(s). Parents are differentiated by "query" and "placed" - where
	 * "placed" is assumed to come from the {@code placedTiles} set, and thus will
	 * not overlap with any other tile in that set. The "query" tile is what we are
	 * currently investigating for overlap, so any children of the query tile must
	 * be added back to the set of tiles that will be used for further overlap
	 * checks.
	 */
	private static void splitOverlappingRegions(Set<ClassifiedRegion> placedTiles, Stack<ClassifiedRegion> rawTiles, ClassifiedRegion queryTile, ClassifiedRegion placedTile) {
		// Tiles are different but overlapping. So we need to identify the start and end
		// points of
		...
		for (int i = 0; i < allIntervals.length; i++) {
			...
			// Which tiles the points belong to doesn't matter. We just need to make
			// intervals of the adjacent points
			...
			if (points.get(0).equals(points.get(1))) {
				...
				if (points.get(2).equals(points.get(3))) {
					...
					// First three points are equal, but last one is different. Single point shares
					// edge
					intervals.add(new Interval(points.get(0)));
					intervals.add(new Interval(points.get(0) + 1, points.get(3)));
				} else {
					// Overlap at first point, other two are unique (partial overlap with shared
					// edge)
					intervals.add(new Interval(points.get(0), points.get(2)));
				...
				// Last 2 points are equal, other two are unique (partial overlap with shared
				// edge)
				intervals.add(new Interval(points.get(0), points.get(1) - 1));
		...
		// Now that we know our potential intervals, we need to enumerate a list of all
		// regions constructable from those intervals
		int[] pos = new int[allIntervals.length];

		// Build and place all regions
		buildAllRegions(allIntervals, pos, 0, queryTile, placedTile, placedTiles, rawTiles);
	}

	/**
	 * Helper class to perform tile processing (iteration through a region, fusion
	 * of input pixels, and population of output pixels). Array fields allow one to
	 * be created per thread, then have values modified externally.
	 */
	private static class TileProcessor<T extends RealType<T>> implements Runnable {
		...
		public TileProcessor(int threadNumber, List<ArrayList<RealRandomAccess<? extends RealType<?>>>> interpolators, ArrayList<? extends ImageInterpolation<? extends RealType<?>>> input,
				Vector<Chunk> threadChunks, int numImages, Img<T> output, PixelFusion fusion, ClassifiedRegion[] currentTile, ArrayList<InvertibleBoundable> transform, ImagePlus[] fusionImp,
		...
		public void run() {
			...
			try {
				...
				processTile(currentTile[0], 0, myFusion, transform, in, out, inPos, threadNumber, count, lastDraw, fusionImp[0]);
		...
		/**
		 * Helper method to lazily initialize the input image interpolators, creating
		 * one list per thread.
		 */
		private ArrayList<RealRandomAccess<? extends RealType<?>>> getThreadInterpolators(List<ArrayList<RealRandomAccess<? extends RealType<?>>>> interpolators, int threadNumber,
				ArrayList<? extends ImageInterpolation<? extends RealType<?>>> input, int numImages) {
		 ...
		 */
		private void processTile(ClassifiedRegion r, int depth, PixelFusion myFusion, ArrayList<InvertibleBoundable> transform, ArrayList<RealRandomAccess<? extends RealType<?>>> in,
				RandomAccess<T> out, double[][] inPos, int threadNumber, int[] count, long[] lastDraw, ImagePlus fusionImp) throws NoninvertibleModelException {
			processTile(r, r.classArray(), depth, myFusion, transform, in, out, inPos, threadNumber, count, lastDraw, fusionImp);
		}

		/**
		 * Helper method to fuse all the positions of a given {@link ClassifiedRegion}.
		 * Since we do not know the dimensionality of the region, we recurse over each
		 * position of each dimension. The tail step of each descent iterates over all
		 * the images (classes) of the given region, fusing the pixel values at the
		 * current position of each associated image. This final value is then set in
		 * the output.
		 */
		private void processTile(ClassifiedRegion r, int[] images, int depth, PixelFusion myFusion, ArrayList<InvertibleBoundable> transform, ArrayList<RealRandomAccess<? extends RealType<?>>> in,
				RandomAccess<T> out, double[][] inPos, int threadNumber, int[] count, long[] lastDraw, ImagePlus fusionImp) throws NoninvertibleModelException {
			...
			if (depth < r.size()) {
				...
				for (int i = start; i < end; i++) {
					...
					processTile(r, images, depth + 1, myFusion, transform, in, out, inPos, threadNumber, count, lastDraw, fusionImp);
					// move forward
					out.fwd(depth);
				}
	 ...
	 * combinations from an arbitrary number of {@link Interval} lists. One list is
	 * required for each dimension of the final regions. The {@code allRegions} list
	 * is then populated with these combinations.
	 * 
	 * @param rawTiles
	 * @param placedTiles
	 */
	private static void buildAllRegions(List<Interval>[] allIntervals, int[] ivalIndices, int depth, ClassifiedRegion queryTile, ClassifiedRegion placedTile, Set<ClassifiedRegion> placedTiles,
			Stack<ClassifiedRegion> rawTiles) {
		if (depth != ivalIndices.length) {
			for (int i = 0; i < allIntervals[depth].size(); i++) {
				buildAllRegions(allIntervals, ivalIndices, depth + 1, queryTile, placedTile, placedTiles, rawTiles);
	 ...
	 * Fuse one slice/volume (one channel)
	 * 
	 * @param outputSlice - same the type of the ImagePlus input, just one slice
	 *                    which will be written to the output directory
	 * @param input       - FloatType, because of Interpolation that needs to be
	 *                    done
	 * @param transform   - the transformation
	 */
	protected static <T extends RealType<T>> void writeBlock(final Img<T> outputSlice, final int numSlices, final int t, final int numTimePoints, final int c, final int numChannels,
			final ArrayList<? extends ImageInterpolation<? extends RealType<?>>> input, final double[] offset, final ArrayList<InvertibleBoundable> transform, final PixelFusion fusion,
		...
		try {
			...
			for (int slice = 0; slice < numSlices; ++slice) {
				IJ.showStatus("Fusing time point: " + t + " of " + numTimePoints + ", " + "channel: " + c + " of " + numChannels + ", slice: " + (slice + 1) + " of " + numSlices + "...");
...
//				fs.saveAsTiff(new File(outputDirectory, "img_t" + lz(t, numTimePoints) + "_z" + lz(slice + 1, numSlices) + "_c" + lz(c, numChannels)).getAbsolutePath());
				fs.saveAsPng(new File(outputDirectory, "img_t" + lz(t, numTimePoints) + "_z" + lz(slice + 1, numSlices) + "_c" + lz(c, numChannels) + ".png").getAbsolutePath());
			}
		} catch (NoninvertibleModelException e) {
			Log.error("Cannot invert model, qutting.");
			return;
	 ...
	 * the output.
	 */
	private static <T extends RealType<T>> void writeTile(ClassifiedRegion r, int depth, final int slice, PixelFusion myFusion, ArrayList<InvertibleBoundable> transform, double[] offset,
			ArrayList<RealRandomAccess<? extends RealType<?>>> in, RandomAccess<T> out, double[][] inPos, int[] count, final long sliceSize, final int numSlices) throws NoninvertibleModelException {
		// NB: there are two process tile methods, one for in-memory fusion
		// and one for writing to disk. They are slightly different, but
		// if one is updated the other should be as well!
		...
		if (depth < out.numDimensions()) {
			...
			for (int i = start; i < end; i++) {
				...
				// Recurse to the next depth (dimension)
				writeTile(r, depth + 1, slice, myFusion, transform, offset, in, out, inPos, count, sliceSize, numSlices);
				out.fwd(depth);
			}
	...
	/**
	 * Estimate the bounds of the output image. If there are more models than
	 * images, we assume that this encodes for more timepoints. E.g. 2 Images and 10
	 * models would mean 5 timepoints. The arrangement of the models should be as
	 * follows:
	 * 
	 * image1 timepoint1 image2 timepoint1 image1 timepoint2 ... image2 timepoint5
	 * 
	 ...
	 * @param offset         - the offset, will be computed
	 * @param size           - the size, will be computed
	 * @param imgSizes       - the dimensions of all input images imgSizes[ image ][
	 *                       x, y, (z) ]
	 * @param models         - all models
	 * @param dimensionality - which dimensionality (2 or 3)
	 */
	public static void estimateBounds(final double[] offset, final int[] size, final int[][] imgSizes, final ArrayList<InvertibleBoundable> models, final int dimensionality) {
		...
		// Log.info( "1: " + Util.printCoordinates( min[ 0 ] ) + " -> " +
		// Util.printCoordinates( max[ 0 ] ) );
		// Log.info( "2: " + Util.printCoordinates( min[ 1 ] ) + " -> " +
		// Util.printCoordinates( max[ 1 ] ) );
		...
		// Log.info( "output: " + Util.printCoordinates( minImg ) + " -> " +
		// Util.printCoordinates( maxImg ) );

		// the size of the new image
		// final int[] size = new int[ dimensionality ];
		// the offset relative to the output image which starts with its local
		// coordinates (0,0,0)
	 ...
	 * lastDraw is past the {@link #redrawDelay}, {@link ImagePlus#updateAndDraw()}
	 * will be invoked on the given image. The return value can be used to keep
	 * track of when the image was last drawn.
	 * 
	...
	private static long drawFusion(final long lastDraw, final ImagePlus fusion) {
		...
		// If enough time has passed, redraw the image and return the time of the latest
		// draw
		...
		// As the image was not drawn, we just should still return the time of the last
		// draw
		return lastDraw;
	}
