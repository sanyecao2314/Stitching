...
public class CollectionStitchingImgLib {
					...
					public void run() {
						...
						for (int i = 0; i < pairs.size(); i++) {
							if (i % numThreads == myNumber) {
								...
								final PairWiseStitchingResult result = PairWiseStitchingImgLib.stitchPairwise(pair.getImagePlus1(), pair.getImagePlus2(), roi1, roi2, pair.getTimePoint1(),
	...
	protected static Vector<ComparePair> findOverlappingTiles(final ArrayList<ImageCollectionElement> elements, final StitchingParameters params) {
		...
		// Added by John Lapage: if the sequential option has been chosen, pair up each
		// image
		...
		for (int i = 0; i < elements.size() - 1; i++)
			for (int j = i + 1; j < elements.size(); j++) {
				...
				if (overlapping) {
					// final ImagePlusTimePoint impA = new ImagePlusTimePoint( e1.open(),
					// e1.getIndex(), 1, e1.getModel().copy(), e1 );
					// final ImagePlusTimePoint impB = new ImagePlusTimePoint( e2.open(),
					// e2.getIndex(), 1, e2.getModel().copy(), e2 );
