...
//	final private String myURL = "http://fly.mpi-cbg.de/preibisch";
	...
	public static int defaultMemorySpeedChoice = 1;

	// Added by John Lapage: user sets this parameter to define how many adjacent
	// files each image will be compared to
	...
	public static int defaultResult = 1;
...
//
//			// Modified by John Lapage: copying the general setup for Unknown Positions
//			// option
...
//		// added by John Lapage: creates text box in which the user can set which range
//		// to compare within. Would be nicer as an Integer.
//		if (gridType == 7)
...
//		final int gridSizeX, gridSizeY;	//x,y方向分别有多少行列
//		double overlapX, overlapY; //重合区域
		int startI = 1, //从几开始,如:0或1
				startX = 0, startY = 0;
...
//		String directory,	//待处理图片存储路径 
		String outputFile = null, seriesFile;
//		final boolean confirmFiles; //选择的图片是否需要弹窗确认
//		final String filenames;	//文件名的格式
...
//		}
		...
		// added by John Lapage: sets the parameters object to recognise that Sequential
		// File pairing should be performed
		if (gridType == 7)
		...
		// open all images (if not done already by grid parsing) and test them, collect
		// information
		int numChannels = -1;
		int numTimePoints = -1;
		...
		// the dimensionality of each image that will be correlated (might still have
		// more channels or timepoints)
...
//			}
			...
			if (is32bit)
				...
				imp = Fusion.fuse(new UnsignedShortType(), images, models, params.dimensionality, params.subpixelAccuracy, params.fusionMethod, noOverlap, false,
						params.displayFusion);
			else if (is8bit)
				imp = Fusion.fuse(new UnsignedByteType(), images, models, params.dimensionality, params.subpixelAccuracy, params.fusionMethod, noOverlap, false,
						params.displayFusion);
			else
...
//				imp.show();
	...
	/**
	 * Generates a ROI for each tile in the list of optimized images. The fusedImage
	 * is the resultant on which the ROIs will be drawn. The offset is a global
	 * offset to 0,0 for the upper leftmost tile.
	 */
	...
	/**
	 * Initializes an {@link ImageReader} if the provided reader is null, or does
	 * not match the given file id.
	 * <p>
	...
	protected ArrayList<ImageCollectionElement> getLayoutFromMultiSeriesFile(final String multiSeriesFile, final double increaseOverlap, final boolean ignoreCalibration, final boolean invertX,
			final boolean invertY, final boolean ignoreZStage, final Downsampler ds) {
		...
		try {
			...
			for (int series = 0; series < numSeries; ++series) {
				...
				for (int t = 0; t < maxT; ++t) {
					double[] location = CommonFunctions.getPlanePosition(r, retrieve, series, t, invertX, invertY, ignoreZStage);
		...
		try {
			...
			for (int series = 0; series < elements.size(); ++series) {
				...
				if (element.getDimensionality() == 2)
					Log.info("series " + series + ": position = (" + element.getOffset(0) + "," + element.getOffset(1) + ") [px], " + "size = (" + element.getDimension(0) + ","
							+ element.getDimension(1) + ")");
				else
					Log.info("series " + series + ": position = (" + element.getOffset(0) + "," + element.getOffset(1) + "," + element.getOffset(2) + ") [px], " + "size = (" + element.getDimension(0)
							+ "," + element.getDimension(1) + "," + element.getDimension(2) + ")");
			}
	 ...
	 * @return 
	 */
	protected ArrayList<ImageCollectionElement> getAllFilesInDirectory(final String directory) {
...
//		final int gridOrder = grid.getOrder();
	...
	protected void getPosition(final int[] currentPosition, final int i, final int gridType, final int gridOrder, final int sizeX, final int sizeY) {
		// gridType: "Row-by-row", "Column-by-column", "Snake by rows", "Snake by
		// columns", "Fixed position"
		// gridOrder:
		// choose2[ 0 ] = new String[]{ "Right & Down", "Left & Down", "Right & Up",
		// "Left & Up" };
		// choose2[ 1 ] = new String[]{ "Down & Right", "Down & Left", "Up & Right", "Up
		// & Left" };
		// choose2[ 2 ] = new String[]{ "Right & Down", "Left & Down", "Right & Up",
		// "Left & Up" };
		// choose2[ 3 ] = new String[]{ "Down & Right", "Down & Left", "Up & Right", "Up
		// & Left" };
